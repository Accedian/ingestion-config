# Cisco PCA Product IOS XE

Product IOS XE telemetry package with the baseline CPU, memory, and interface
objects plus the promoted BGP neighbor, environment sensor, and PTP objects.

The supplemental objects and their live-validation evidence were promoted from
the IOS XE community package into this product pack.
...
- `telemetry-collector-configuration/input-cisco-telemetry-xe-mdt.conf`: product collector config with the baseline plus supplemental BGP/environment/PTP additions.
- `telemetry-collector-configuration/transformation-cisco-telemetry-xe-bgp-neighbor.conf`: BGP shaping block.
- `telemetry-collector-configuration/transformation-cisco-telemetry-xe-environment-sensor.conf`: environment-sensor shaping block.
- `telemetry-collector-configuration/transformation-cisco-telemetry-xe-ptp.conf`: PTP shaping block.
- `sensor-collector-configuration/cisco-telemetry-xe.yaml`: job list for all product XE objects.
...
Threshold leaves are intentionally ignored in this supplemental model so the dashboard receives one aggregated environment object per router instead of one object per physical sensor.
...
This package is now part of the PCA product XE baseline.
...
3. Re-test after the final production packaging path is deployed.
