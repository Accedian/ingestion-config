...
def to_camel_case(value):
    words = value.split("_")
    return first_lower(words[0]) + "".join(first_upper(word) for word in words[1:])

def first_upper(my_string):
    if my_string:  # Check if the string is not empty
        customized_capitalized_string = my_string[0].upper() + my_string[1:]
        return(customized_capitalized_string)
    return(my_string)

def first_lower(my_string):
    if my_string:  # Check if the string is not empty
        customized_capitalized_string = my_string[0].lower() + my_string[1:]
        return(customized_capitalized_string)
    return(my_string)
...
def clean_analytics_name(name, remove_prefix=False):
    # Remove trailing known suffix values
    ...
    if output_file:
        ...
        try:
            with open(input_file, "w") as f:
                json.dump(data, f, indent=2)
            print(f"Processing complete! Updated JSON written to '{input_file}'")
        except Exception as e:
            print(f"Error writing to output file: {e}")
            sys.exit(1)
